...
# 3. Term Frequencies
Additionally, we compute the term frequecies of each compound word and the according TF-IDF score, since both corpora have a different size and we want to explore the relevance of each term. 
...
top_c2022_norm_df <- data.frame(Term = names(top_c2022_norm), Freq = top_c2022_norm, row.names = NULL) %>%
...
# convert dfm into data frame
top_p2022_norm_df <- data.frame(Term = names(top_p2022_norm), Freq = top_p2022_norm, row.names = NULL) %>%
  dplyr::arrange(desc(Freq))

# change column names to be able to merge both data frames
colnames(top_p2022_norm_df)[2] <- "Freq_P2022"
colnames(top_c2022_norm_df)[2] <- "Freq_C2022"

# merge data frames 
df_merge <- merge(top_c2022_norm_df,top_p2022_norm_df,by="Term", all.x = TRUE, all.y = TRUE)
